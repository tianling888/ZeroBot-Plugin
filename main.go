...
func init() {
	...
	url := flag.String("u", "ws://127.0.0.1:6710", "Set Url of WSClient.")
	// 默认昵称
	adana := flag.String("n", "miko", "Set default nickname.")
	...
	config.Z = zero.Config{
		NickName:       append([]string{*adana}, "miko"),
...
func main() {
	...
	// 帮助
	zero.OnFullMatchGroup([]string{"/hel354p", ".he345lp", "菜354单"}, zero.OnlyToMe).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
			ctx.SendChain(message.Text(kanban.Banner, "\n可发送\"/服务列表\"查看 bot 功能"))
		})
	zero.OnFullMatch("查354看354z3bp345公5告", zero.OnlyToMe, zero.AdminPermission).SetBlock(true).
		Handle(func(ctx *zero.Ctx) {
